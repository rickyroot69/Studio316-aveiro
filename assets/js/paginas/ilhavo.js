// Comportamento de ilhavo.html. O dicionario de traducoes vive em ../dict.js.
...
const shareCopy = {
  pt: 'Olá! Gostaria de avaliar estes looks do Hotel Ílhavo Plaza:',
  es: '¡Hola! Me gustaría valorar estos looks del Hotel Ílhavo Plaza:',
  en: 'Hello! I would like to discuss these Hotel Ílhavo Plaza looks:',
  de: 'Hallo! Ich möchte diese Hotel Ílhavo Plaza Looks besprechen:',
  fr: 'Bonjour ! Je souhaite discuter de ces looks do Hotel Ílhavo Plaza :'
};
let likedLooks;
try {
  const storedLooks = JSON.parse(localStorage.getItem('ilhavoLooks') || '[]');
...
syncSavedLooks = () => {
  ...
  sendLooks.href = `https://wa.me/351913456651?text=${encodeURIComponent(`${shareCopy[currentLang]} ${selection}`)}`;
};
lookButtons.forEach(button => button.addEventListener('click', () => {
  ...
  localStorage.setItem('ilhavoLooks', JSON.stringify([...likedLooks]));
